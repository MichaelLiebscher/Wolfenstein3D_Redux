...
/**
 * \brief Entity is going to move in a new direction. 
 * \param[in] self Valid pointer to entity_t structure of entity to change
 * \param[in] new_dir New direction
...
/**
 * \brief Checks a straight line between player and current object.
 * \param[in] self Valid pointer to entity_t structure of entity to check against
...
/**
 * \brief Called by entities that are not chasing the player. 
 * \param[in] self Valid pointer to entity_t structure of entity to check against
...
PRIVATE _boolean AI_FindTarget( entity_t *self )
{
	if( self->reacttime ) // count down reaction time
	{
		...
		self->reacttime = 0; // time to react
	}
	else
	{
...
/**
 * \brief Moves object for distance in global units, in ob->dir direction. 
...
PUBLIC void T_Path( entity_t *self )
{
	...
	if( !self->speed ) 
	{
		return; // if patrolling with a speed of 0
	}
...
//	Sound_StartSound( NULL, 1, CHAN_VOICE, Sound_RegisterSound( "lsfx/076.wav" ), 1, ATTN_NORM, 0 );  //gsh this was the original code
	...
	if( dx <= MINACTORDIST )
	{
		dy = ABS( Player.position.origin[ 1 ] - self->y ) - TILE_GLOBAL;
		if( dy <= MINACTORDIST )
		{
			if(US_RndT()<180)
			{
...
//
	dx = ABS( Player.position.origin[ 0 ] - self->x ) - TILE_GLOBAL / 2;
	if(dx <= MINACTORDIST)
	{
		dy = ABS( Player.position.origin[ 1 ] - self->y ) - TILE_GLOBAL / 2;
		if( dy <= MINACTORDIST )
		{
			A_StateChange( self, st_shoot1 );
			return; // bite player!
...
// So left only check if guard is in player's fov: FIXME: not fixed fov!
 	if( angle_diff( TransformPoint( self->x, self->y, Player.position.origin[0], Player.position.origin[1] ), RAD2DEG( Player.position.angle ) ) < (M_PI/3) )
	{
		hitchance -= dist * 16;
	}
	else
